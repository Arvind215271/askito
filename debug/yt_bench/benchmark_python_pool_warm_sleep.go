...
func BenchmarkPythonPoolWarmSleep(ctx context.Context, playlistID string, outputDir string, workers int) (BenchmarkResult, error) {
	...
	logFile, err := os.Create(filepath.Join(logDir, fmt.Sprintf("python_pool_warm_sleep_%d.log", workers)))
	if err != nil {
